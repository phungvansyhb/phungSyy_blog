...
async function uploadImageAndReturnUrl(file: File):Promise<string> {
    return uploadBytes(storageRef(file.name), file).then((snapshot) => {
...
export { createDoc, getDetailDoc, getListDocs, updateDocument, deleteDocument, writeBatchDoc, uploadImageAndReturnUrl }